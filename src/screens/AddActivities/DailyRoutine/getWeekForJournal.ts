...
const getWeekForJournal = (date: moment.Moment) => {
  const selectedDate = moment(date);
  let days: any[] = [];

  let weekStart_ = selectedDate.clone().subtract(6, "days");

  for (let i = 0; i <= 6; i++) {
    let dayDate = moment(weekStart_).add(i, "days");
    let day = {
...
export default getWeekForJournal;
