...
const AffirmationCard: React.FC<AffirmationCardProps> = ({
  ...
  return (
    <ImageBackground
      ...
      resizeMode="stretch"
      source={imagePath?.bg_aff}
    >
      <View style={stylesInline?.position}>
        <Text style={stylesInline.overlayText}>{"Today's affirmation"}</Text>
        <Text style={stylesInline.suboverlayText}>
          {"Power up with a daily dose of positivity"}
        </Text>
        <TouchableOpacity
        ...
        >
          ...
          />
          <Text style={stylesInline.upgradeText}>{"Tap to feel better"}</Text>
