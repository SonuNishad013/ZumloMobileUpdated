...
import { getShortTermGoal } from "../../../../redux/selector";
...
const ShortTermGoals = ({
  ...
  const [Alldata, setAlldata] = useState<any>(inputsData);
  const goalData: any = useSelector(getShortTermGoal());
  ...
  const addMoreFields = () => {
    ...
    setShowDatePicker([...showDatePicker, false]); // Add a new false entry for the new field
  };
  // const onDeleteFields = (index: any) => {
  //   setInputsData(inputsData.filter((_, i) => i !== index)); // Remove item by index
  // };
  const onDeleteFields = (index: number) => {
    const newInputsData = inputsData.filter((_, i) => i !== index);
    setInputsData(newInputsData);

    const newShowDatePicker = showDatePicker.filter((_, i) => i !== index);
    setShowDatePicker(newShowDatePicker); // Update the visibility array
  };
  const onNext = () => {
  ...
  let tmrwDate = moment(new Date()).add(1, "days");
  console.log("check date-=-=>=>", moment(tmrwDate).toString(), new Date());
    ...
    return (
      <View key={index}>
        ...
        <CustomDatePicker
          ...
          pickerTitle={"Select date (From today to 3 months)"}
          onCancelClick={() => {
            handleDatePickerVisibility(index, false);
          }}
          isMaxDate
          MaxDate={
            new Date(
              moment(getDateThreeMonthsLater().toDateString()).format(
                "YYYY-MM-DD"
              )
            )
          }
    ...
    <View style={{ flex: 1 }}>
      ...
      >
        <View style={{ flex: 1 }}>
          ...
          <TitleHeader
            title={"What short-term goal do you want to focus on? "}
          />
