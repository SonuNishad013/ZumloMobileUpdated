...
const DropdownComponent: <T>(
  ...
  React.forwardRef((props, currentRef) => {
    ...
    const {
      ...
      dropdownPosition = "auto", //top,auto
      flatListProps,
      searchQuery,
      backgroundColor,
      onChangeText,
    ...
    const updateListData = () => {
      if (multiSelected) {
        ...
        for (let elem of listData) {
          data.push({
            ...elem,
            isSelected: selectedValueArray?.some(
              (item: any) => item == elem.title
            ),
          });
        }
        setListData(data);
    ...
    const onSelect = useCallback(
      (item: any, index: number) => {
        ...
        if (closeModalWhenSelectedItem) {
          ...
          setSearchText("");
          onSearch("");
          if (multiSelected == false) eventClose();
        }
      },
      [
        confirmSelectItem,
    ...
    const _renderDropdown = () => {
      let isSelected = currentValue && _get(currentValue, valueField);
      let title = "";
      if (multiSelected) {
        title = listData
          ...
          .map((elem) => elem.title)
          .join(", ");
        console.log("title title _renderDropdown --> ", JSON.stringify(title));
      ...
      return (
        ...
        >
          <View>
            {LabelName && (
              <Text style={{ color: colors.grey, fontSize: textScale(10) }}>
                {LabelName}
              </Text>
            )}
            <View style={styles.dropdown}>
              ...
              <Text
                style={[
                  styles.textItem,
                  title != placeholder ? selectedTextStyle : placeholderStyle,
                  font(),
                  { height: "auto" },
                ]}
                {...selectedTextProps}
    ...
    const _renderItem = useCallback(
      ({ item, index }: { item: any; index: number }) => {
        ...
        if (multiSelected) {
          ...
          selected = _isEqual(_get(item, valueField), isSelected);
